...
% MORE THAN ONE VEHICLE RUNNING SUCCESSFULLY
...
for i=1:sim_time
    for j=1:length(R2_ini(1,:))
        ...
        if s(i,j)>=430
            s(i+1,j) = v1(i,j)*i+(0.5*a_model*i^2);
            v1(i+1,j) = v1(i,j)+a_model*(i-tme_sr)
            x_model(i,j) = 430+v1(i,j)*(i-tme_sr)+(0.5*a_model*(i-tme_sr)^2);
            y_model(i,j) = 107.625; 
        end
...
%             v1(i+1,j) = sqrt((vf_model(1,j)).^2-(2*a_model*s(i,j)));
%             x_model(i,j)=430+v1(i+1,j)*(i-tme_sr(j))+(0.5*a_model*(i-tme_sr(j)).^2);
%             p=v1(i+1,j)*(i-tme_sr(j))+(0.5*a_model*(i-tme_sr(j)).^2)
%             y_model(i,j)=107.625; 
%             t1(i)=i*1;