...
% the vehicles reaching teh end of the control zone at velocity in range of
% standard deviation sigma=2 m/s

dt=1; %time step is 1 second
vmax_2 = 13.4112; % [m/s] ==> 30 MPH
vmax_mr = 13.4112; % [m/s] ==> 30 MPH
vmax_sr = 13.4112;   % mr= main raod, sr= secondary road
vavg_mr = 13.4112; % [m/s] ==> 30 MPH
vavg_sr = 13.4112; % 30 MPH %VELOCITY MEASURED BY RSU1=MEAN
sigma=2; % [m/s]
N=1000; % NUMBER OF VALUES IN GAUSSIAN DISTRIBUTION
vmin_mr = 0; % [m/s] 
vmin_sr = 0;
vmax = 30; 
...
numel = t_sim/dt; %======number of steps 
...
% vo: Initial speed
R2_ini(5,:)= ones(1,length(R2_xo))*vavg_sr; %===NEEDS CHANGE
% vf: final speed
R2_ini(6,:)= ones(1,length(R2_xo))*vmax_sr; 
% tf: time to reach intersection
R2_ini(7,:)= RSU1(sigma,vavg_sr,cz_length,N,R2_xo); 
