...
% v2_preidct=sqrt(v1_model(1,1).^2+2*a_predict*(100)) % vel at RSU2 predicted using acclf from gaussain
% tm_predict=2*(400)/(vf_gaussian+v1_model(1,1))                                % 100 is distance between two RSUs
    ...
    for i2=1:length(R1_ini_h(2,:))        
        ...
        if x(i1-1,i2)>=0 && x(i1-1,i2)<=400 
            if R1_ini_h(1,:)==1 %choosing only mr vehcles
                ...
                vf(i1,i2) = vf(i1-1,i2);
                to(i1,i2) = t(i1-1,i2);
                tf(i1,i2) = tm_predict; % will change based on RSU 2 update
...
%         else
    ...
    if i==tm_100 && error >0.3
        tm_predict=2*300/(vf_newpredict+v2_RSU)-t_headway;
    else
%    vf_consider = vf_model %then the initial predicted can be there as
                            % it does not make much difference
        tm_predict=tm_predict-t_headway;
    end
end
